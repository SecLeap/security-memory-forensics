...
| --- | --- | --- | --- |
| 本地模拟 | Win11 上 FakeNet-NG | 快速观察单机的 DNS/HTTP/其他协议访问尝试 | 不同时使用 Ubuntu 的 FakeDNS、Apache 或 INetSim 解释同一端口/会话 |
| 外部模拟 | Ubuntu FakeDNS + Apache 或 INetSim | 需要独立服务端日志、可控响应或跨主机 PCAP | 不启动 FakeNet-NG 的流量拦截/服务模拟 |
...
2. 关闭 Ubuntu 模拟服务，或选择外部模拟模式而不启动 FakeNet-NG。
...
1. 选择“本地模拟”模式：Win11 只保留内部网卡，Ubuntu 的 FakeDNS/Apache/INetSim 全部停止。
...
| 与 Ubuntu 日志不一致 | 是否误混用了本地/外部模拟模式、时间/时区 | 每次实验只选一个主模拟模式，统一 UTC 时间线 |
